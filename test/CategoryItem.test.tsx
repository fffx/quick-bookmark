...
import type { CategoryItemProps } from "../source/Popup/CategoryItem";

describe("CategoryItem Component", () => {
  const mockNode = {
    id: "1",
    title: "Test Folder",
    children: [{ id: "2", title: "Bookmark 1", url: "https://example.com" }],
  ...
  it("should show focused state", () => {
    const { container } = render(
      <CategoryItem {...mockProps} focused={true} />,
    );
  ...
  it("should show containsCurrentTab state", () => {
    ...
    const { container } = render(
      <CategoryItem {...mockProps} node={nodeWithTab} />,
    );
  ...
  it("should render NEW folder correctly", () => {
    ...
    const { container } = render(
      <CategoryItem {...mockProps} node={newNode} />,
    );
  ...
  it("should call create bookmark on NEW folder click", async () => {
    ...
    global.browser.bookmarks.create.mockResolvedValue({
      ...newNode,
      id: "999",
    });
  ...
  it("should render with titlePrefix", () => {
    ...
    expect(
      screen.getByText(/Root \/ Parent \/ Test Folder/),
    ).toBeInTheDocument();
  });
});
