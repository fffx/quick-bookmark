...
import type { PerfConfig } from "./performance.config";
...
function generateBookmarkTree(config: PerfConfig) {
  let id = 0;
  const nextId = () => String(id++);
  const root: TestNode = { id: nextId(), title: "", children: [] };
  let folderCount = 0;
...
function pickQuery(tree: ReturnType<typeof generateBookmarkTree>["tree"]) {
...
async function sampleRender(config: PerfConfig) {
...
async function sampleFilter(
  config: PerfConfig,
  tree: ReturnType<typeof generateBookmarkTree>["tree"],
) {
...
async function medianOf<T extends { ms: number }>(
  config: PerfConfig,
  run: () => Promise<T>,
) {
  const samples: T[] = [];
